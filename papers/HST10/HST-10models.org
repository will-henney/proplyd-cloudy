...
| O I       | 5577 | 0.24 +/- 0.06    | 0.00 +/- 0.00    | 0.74 +/- 0.06    | 0.70 +/- 0.16    |
...
#+header: :results file
#+BEGIN_SRC python :var datatab=flux-comparison-table
import numpy as np
...
fluxes, errors = dict(p=dict(), b=dict()), dict(p=dict(), b=dict())
for sample in "pb":
    for person in "wy":
        fluxes[sample][person] = np.array([f[0] for f in fluxdict[sample][person]])
        errors[sample][person] = np.array([f[1] for f in fluxdict[sample][person]])

sample = "p"
# # make the plots
plt.loglog(fluxes[sample]["w"], fluxes[sample]["y"], ".")
plt.errorbar(fluxes[sample]["w"], fluxes[sample]["y"],
             xerr=errors[sample]["w"], yerr=errors[sample]["y"], fmt=None) 

dx, dy, i = -20, 20, 0
for label, x, y in zip(labels, fluxes[sample]["w"], fluxes[sample]["y"]):
    plt.annotate(
        label, 
        xy = (x, y), xytext = (dx, dy),
...
plt.xlabel("Will")
plt.ylabel("Yiannis")
plt.title("Battle of the fluxes!")
